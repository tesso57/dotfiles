...
## 10. As-built 補足 (実装後追記)

実装中の検証で当初設計から下記に変更:

- **organizeImports は vtsls ではなく conform 側に移管**: `vim.lsp.buf.code_action({apply=true})` は async で `BufWritePre` 中に完了しないため、保存ファイルに反映されないレースが発生。`conform` の `biome-check` (`biome check --write --unsafe`) に統合し、format + import 並び替え + 未使用 import 削除 + lint 自動修正を1パスでこなすようにした。
- **biome-check の args を上書き**: conform 同梱の `biome-check` formatter は default で `--unsafe` を付けない。`noUnusedImports` の auto-fix を効かせるため `formatters["biome-check"].args` で `--unsafe` を含む完全な args を上書き。
- **`format_on_save.timeout_ms` を 500ms → 2000ms に変更**: biome-check は lint パスを伴うため余裕を持たせた。

これに伴い、§3.1#1 で予定していた lsp.lua の `BufWritePre` (organizeImports) は実装で削除されている。inlay hint 設定と `<leader>ih` のキーマップは設計どおり残っている。
